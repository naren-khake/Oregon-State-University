...
class MCTS:
    ...
    def parallel_roll_outs(self, num_rollouts: int):
        results = []
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(self.single_rollout) for _ in range(num_rollouts)]
    ...
    def single_rollout(self):
        ...
        return node, state.to_play, outcome

    def search(self, time_limit: int):
        start_time = time.process_time()
        num_rollouts = 0
        while time.process_time() - start_time < time_limit:
            rollouts = self.parallel_roll_outs(10)  # Perform 10 rollouts in parallel
            for node, turn, outcome in rollouts:
                self.back_propagate(node, turn, outcome)
            num_rollouts += 10
